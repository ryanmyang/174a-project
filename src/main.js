...
const panels = [];
const pinkMetalMaterial = new THREE.MeshStandardMaterial({ //pink metallic for rails and platform
  color: 0xff6b81,
  metalness: 0.8,  
  roughness: 0.2, 
});
...
for (let i = 0; i < strips.length - 1; i++) {
  ...
  const top1 = new THREE.Vector3(strip1.position.x, strip1.position.y + stripLength / 2, strip1.position.z);
  const top2 = new THREE.Vector3(strip2.position.x, strip2.position.y + stripLength / 2, strip2.position.z);
  ...
  const midpoint = new THREE.Vector3().addVectors(top1, top2).multiplyScalar(0.5); // get the midpoint to position the strip
